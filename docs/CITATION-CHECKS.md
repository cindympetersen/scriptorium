...
> **Status: proposed, not built** (drafted 2026-09-13). The desk today has
> `check_scripture.py` and `check_quotes.py` as described under *How it works now*.
> `check_loci.py`, the canon record, `--scheme verse` and the `NO CANON INDEX` finding
> are this design.
...
**`check_scripture.py`** checks a draft's scripture quotations — body and footnotes —
...
`check_scripture.py` owns this one."*
...
- `check_scripture` never sees it. Its locus regex is built from `KJV_BOOKS`, a closed set
...
uses. For the King James that gap was closed a year of pieces ago: `check_scripture`
...
`PAGE OUT OF RANGE`, and why `check_scripture` needs none of it. A citation that survives a
...
- `check_scripture.py` → **`check_loci.py`** — the checker for quotations cited by
  canonical locus.
- `refindex.py --scheme kjv` → **`--scheme verse`**, with `kjv-text` becoming
  `verse-text`. The scheme is "one row per verse"; the King James is one canon it is run
  on.

Nothing about the mechanism is Christian, and naming it for one canon is what kept four
traditions out of it.

**The house conventions stay in the checker**, not in the canon. Bracketed pronoun
substitution and deity-pronoun recasing are how *this house* quotes any sacred text — they
would apply to a Gita quotation identically. They were written down beside the King James
because it was the only canon the tool had.
...
This is the boundary `check_scripture.py` already draws for itself, in its own comment:
...
`check_scripture` is named in 26 files. They divide in two, and the division matters:
...
block (*"by: Eric (authorised 2026-09-10), on check_scripture.py's mechanical pass"*), every
...
1. ~~**Fix `aliases()`**~~ — done 2026-09-13, and it turned out to be the larger half of
   the silence. See above.
2. **Widen `CITATION_SIGNAL`** so a canon citation whose source is *not* held reports
   `NOT HELD` instead of vanishing. Small, and independent of everything below.
3. **Rename** `check_scripture.py` → `check_loci.py` and `--scheme kjv` → `--scheme verse`;
   sweep the 26 files minus the records.
4. **The canon record**, with `kjv.yaml` reproducing today's behavior exactly — `check_loci`
   green on the corpus before and after is the proof.
5. **`NO CANON INDEX`**, once more than one canon is declared.
6. **Indexes per canon**, easiest first.

**Do not run this alongside the shelf migrations.** `REFERENCE-SHELF.md` has its own two
table-rewriting passes, and the proof that each worked is a green `references.py check`.
Three migrations converging on one green run is how a green run stops meaning anything.
Finish one track, then start the other.
